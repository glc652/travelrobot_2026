...
while not app.need_exit():
    ...
    # YOLO识别 - 置信度阈值 - 重叠框阈值
    objs = detector.detect(img, conf_th = YOLO_conf, iou_th = YOLO_iou)
    ...
    for obj in objs:
        ...
        # 误识别多个目标取置信度最高的
        if label in current_frame_results:
            if score < current_frame_results[label]["score"]:
                continue # 跳过当前目标，保留更信赖的目标
