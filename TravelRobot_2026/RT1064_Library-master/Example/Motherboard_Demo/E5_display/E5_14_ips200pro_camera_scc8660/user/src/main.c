...
ips200pro_image_line_uint8_struct border1[SCC8660_H];
ips200pro_image_line_uint8_struct border2[SCC8660_H];

// SCC8660_DATA_FORMAT_DEF 宏定义务必设置为1
// SCC8660_DATA_FORMAT_DEF 宏定义务必设置为1
// SCC8660_DATA_FORMAT_DEF 宏定义务必设置为1
uint16 i = 0;
...
int main(void)
{
    ...
    // 初始化屏幕并创建一个页面
    page_id[0] = ips200pro_init("凌瞳测试", IPS200PRO_TITLE_BOTTOM, 30);
    ...
    camera_id = ips200pro_image_create(40, 20, SCC8660_W, SCC8660_H);
    ...
    while (1)
    {
        if(scc8660_finish_flag)
        {
            ...
            for(i = 0; SCC8660_H > i; i++)
            {
                // 计算左边界
                border1[i].x = (SCC8660_H - i) * temp_start / SCC8660_H;
                border1[i].y = (uint8)i;

                // 计算右边界
                border2[i].x = SCC8660_W - ((SCC8660_H - i) * temp_start / SCC8660_H);
                border2[i].y = (uint8)i;
            }
            temp_start++;
            if(70 < temp_start)
            ...
            ips200pro_image_draw_line(camera_id, 1, border1, SCC8660_H, IMAGE_LINE_TYPE_UINT8, RGB565_BLUE);
            ips200pro_image_draw_line(camera_id, 2, border2, SCC8660_H, IMAGE_LINE_TYPE_UINT8, RGB565_GREEN);

            rectangle_x++;
            if((SCC8660_W - 30) < rectangle_x)
            {
                rectangle_x = 0;
            }
            ...
            ips200pro_image_display(camera_id, scc8660_image[0], SCC8660_W, SCC8660_H, IMAGE_RGB565, 0);
        }
    }
}
