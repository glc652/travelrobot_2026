...
// 
// 2.摄像头接在主板的摄像头接口 注意线序ips 1.14模块插入主板屏幕接口
//  
...
int main(void)
{
    ...
    ips114_init();
    ips114_show_string(0, 0, "scc8660 init.");
    while(1)
    {
        if(scc8660_init())
		{
			ips114_show_string(0, 16, "scc8660 reinit.");
    ...
    while(1)
    {
        ...
        {
            ips114_displayimage8660((const uint16 *)scc8660_image, 240, 135);   // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
