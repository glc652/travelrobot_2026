...
// 接入蓝牙CH9141转串口模块
//      模块管脚            单片机管脚
//      RX                  查看 zf_device_bluetooth_ch9141.h 中 CH9141_RX_PINx 宏定义 默认 D16
//      TX                  查看 zf_device_bluetooth_ch9141.h 中 CH9141_TX_PINx 宏定义 默认 D17
//      RTS                 查看 zf_device_bluetooth_ch9141.h 中 CH9141_RTS_PINx 宏定义 默认 D26
//      GND                 核心板电源地 GND
//      3V3                 核心板 3V3 电源
// *************************** 例程使用步骤说明 ***************************
...
// 3.在逐飞助手上位机中，选择图像传输。
//
// 4.选择蓝牙串口对应的串口号，波特率(默认115200)，点击连接
//
// 5.等待几秒钟，图像就会显示在逐飞串口助手中。


// *************************** 例程测试说明 ***************************
// 1.连接好模块和核心板后，蓝牙的USB端插到电脑上（尽量使用配套主板测试以避免供电不足的问题） 烧录本例程 按下复位后程序开始运行
//
// 2.如果模块未能正常初始化 核心板上的LED1灯会闪烁 一般情况下重试会成功
//
// 3.打开逐飞助手上位机，选择图像传输，打开蓝牙串口的串口号，连接可以看到图像显示
//
// 4.使用115200波特率。逐飞助手的图像大概5帧，这是由于串口传输慢导致的，并非摄像头采集问题。
//
// 5.115200波特率一秒钟大约传输11.25KB数据，一副图像160x120的分辨率大约有2.4KB的数据。
//
...
int main (void)
{
    ...
    // 设置逐飞助手使用DEBUG串口进行收发
    seekfree_assistant_interface_init(SEEKFREE_ASSISTANT_CH9141);
