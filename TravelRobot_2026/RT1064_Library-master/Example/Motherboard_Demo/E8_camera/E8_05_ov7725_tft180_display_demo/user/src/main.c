...
// 接入1.8寸TFT模块
//      模块管脚            单片机管脚
//      SCL                 查看 zf_device_tft180.h 中 TFT180_SCL_PIN 宏定义 默认 B0
//      SDA                 查看 zf_device_tft180.h 中 TFT180_SDA_PIN 宏定义 默认 B1
//      RES                 查看 zf_device_tft180.h 中 TFT180_RES_PIN 宏定义 默认 B2 
//      DC                  查看 zf_device_tft180.h 中 TFT180_DC_PIN 宏定义 默认 C19
//      CS                  查看 zf_device_tft180.h 中 TFT180_CS_PIN 宏定义 默认 B3 
//      BL                  查看 zf_device_tft180.h 中 TFT180_BL_PIN 宏定义 默认 C18
//      GND                 核心板电源地 GND
//      3V3                 核心板 3V3 电源
...
int main(void)
{
    ...
    tft180_init();
    tft180_show_string(0, 0, "ov7725 init.");
    while(1)
    {
        if(ov7725_init())
            tft180_show_string(0, 16, "ov7725 reinit.");
        else
            break;
    ...
    while(1)
    {
        ...
        {
            tft180_displayimage7725((const uint8 *)ov7725_image_binary, 160, 128);
//            tft180_show_binary_image(0, 0, (const uint8 *)ov7725_image_binary, OV7725_W, OV7725_H, 160, 128);
