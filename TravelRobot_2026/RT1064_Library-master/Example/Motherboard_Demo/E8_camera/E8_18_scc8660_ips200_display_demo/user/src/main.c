...
int main(void)
{
    ...
    ips200_show_string(0, 0, "scc8660 init.");
    while(1)
    {
        if(scc8660_init())
		{
			ips200_show_string(0, 16, "scc8660 reinit.");
    ...
    while(1)
    {
        ...
        {
            ips200_displayimage8660((const uint16 *)scc8660_image, 320, 240);   // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
