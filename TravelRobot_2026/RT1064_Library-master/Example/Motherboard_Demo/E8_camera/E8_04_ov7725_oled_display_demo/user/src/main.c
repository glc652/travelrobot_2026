...
// 
// 2.摄像头接在主板的摄像头接口 注意线序OLED模块插入主板屏幕接口
// 
...
int main(void)
{
    ...
    oled_init();
    oled_show_string(0, 0, "ov7725 init.");
    while(1)
    {
        if(ov7725_init())
            oled_show_string(0, 1, "ov7725 reinit.");
        else
            break;
    ...
    while(1)
    {
        ...
        {
            oled_displayimage7725((const uint8 *)ov7725_image_binary);          // 我们不建议使用 oled 查看图像 缩放效果显示效果极差 建议使用 TFT/IPS 系列屏幕
//            oled_show_binary_image(0, 0, (const uint8 *)ov7725_image_binary, OV7725_W, OV7725_H, 128, 64);
